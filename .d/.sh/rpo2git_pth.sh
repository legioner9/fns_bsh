...
fn_vewthhg5231454gfvsXXX() {
	...
	local repo_all_arr_234543htrbgXXX=(aer_foe edu_clng edu_dta_001 edu_dta_002 edu_lnx tml_a cmn_lex fns_bsh git_flw)
	local repo_any_arr_234543htrbgXXX=(edu_clng edu_dta_001 edu_dta_002 edu_lnx tml_a cmn_lex fns_bsh git_flw)

	# local repo_all_arr_234543htrbgXXX=(aer_foe edu_clng edu_lnx tml_a cmn_lex fns_bsh git_flw)
	# local repo_any_arr_234543htrbgXXX=(edu_clng edu_lnx tml_a cmn_lex fns_bsh git_flw)

	cd ${DDir}
	check_repo_adcae_12344XXX() {

		if [[ ! -d $HOME/$adcae_12344XXX ]]; then
			echo -e "${ECHO_WARN}in file://$HOME\aer_foe\.d\.sh\rpo2git.sh , line=${LINENO} edu_lnx REPO_NOT_EXIST return 1${NRM}" >&2
			cd ~
			git clone git@gitflic.ru:legioner9/$adcae_12344XXX.git
			cd $HOME/$adcae_12344XXX
			git remote add gh git@github.com:legioner9/$adcae_12344XXX.git
			git remote add gf git@gitflic.ru:legioner9/$adcae_12344XXX.git
	...
	for item in ${repo_all_arr_234543htrbgXXX[@]}; do
		l_00_echo_code $item
	done

	l_00_echo_info "STP:: add remote to ANY rpo"
	unset IFS
	local item_132456125XXX=
	for item_132456125XXX in ${repo_any_arr_234543htrbgXXX[@]}; do

		l_00_echo_info "--- treat $item_132456125XXX ---"
		adcae_12344=$item_132456125XXX
		check_repo_adcae_12344XXX
	...
	start_all_rpo2git_32eggsdXXX() {
		if [[ -f $HOME/$nm_rpo2git_2345ffXXX/.d/.sh/rpo2git.sh ]]; then
			. $HOME/$nm_rpo2git_2345ffXXX/.d/.sh/rpo2git.sh
		else
			echo -e "${ECHO_WARN}in file://$HOME\$nm_rpo2git_2345ffXXX\.d\.sh\rpo2git.sh , line=${LINENO}  EXEC : '[[ -f $HOME/$nm_rpo2git_2345ffXXX/.d/.sh/rpo2git.sh ]]' in $(pwd), 'RESUME :: $HOME/$nm_rpo2git_2345ffXXX/.d/.sh/rpo2git.sh IS_NOT file' return 1${NRM}" >&2
	...
	local nm_rpo2git_2345ffXXX=

	for item4 in ${repo_any_arr_234543htrbgXXX[@]}; do
		l_00_echo_code "in ${item4}"
		nm_rpo2git_2345ffXXX=${item4}
		start_all_rpo2git_32eggsdXXX
	done

	add_local_repo_56277647XXX() {
		...
		if [[ -d "$part_bare/" ]]; then
			...
			local item2=
			for item2 in ${repo_all_arr_234543htrbgXXX[@]}; do
			...
			local item3=
			for item3 in ${repo_all_arr_234543htrbgXXX[@]}; do
