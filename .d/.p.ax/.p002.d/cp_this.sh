...
fn_5746_9273() {
	...
	local fl_pth_fn_5746_9273="${HOME}/fns_bsh/.d/.p.ax/.p002.d/cp_this.sh"
	local dr_pth_fn_5746_9273=$(dirname ${HOME}/fns_bsh/.d/.p.ax/.p002.d/cp_this.sh)
	local fl_nm_fn_5746_9273=$(basename ${HOME}/fns_bsh/.d/.p.ax/.p002.d/cp_this.sh)
	local rnd=5746_9273
	...
	if [[ -d $(eval "echo \$arg_1_fn_${rnd}")/XXX.fn ]]; then
		echo -e "${ECHO_RET1}in file://$(eval "echo \$fl_pth_fn_${rnd}") , line=${LINENO} DR_EXIST : '[[ -d $(eval "echo \$arg_1_fn_${rnd}")/XXX.fn ]]', return 1${NRM}" >&2
		return 1
	fi

	cp -r $(eval "echo \$prnt1_dr_pth_fn_${rnd}")/XXX.fn $(eval "echo \$arg_1_fn_${rnd}")
