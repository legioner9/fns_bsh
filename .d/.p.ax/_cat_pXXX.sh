...
fn_15625_16260() {
	...
	local fl_pth_fn_15625_16260="${HOME}/fns_bsh/.d/.p.ax/_cat_pXXX.sh"
	local dr_pth_fn_15625_16260=$(dirname ${HOME}/fns_bsh/.d/.p.ax/_cat_pXXX.sh)
	local fl_nm_fn_15625_16260=$(basename ${HOME}/fns_bsh/.d/.p.ax/_cat_pXXX.sh)
	local rnd=15625_16260
	...
	[[ "$1" == "-h" ]] && {
        ...
        doing :: cat [grep "doing ::"] all ~/fns_bsh/.d/.p.ax/.p[***].d/g.pr
			\$1 - 
			\$2 - 
			\$3 - 
            exa use ::
	...
	local dr_nm=
	for dr_nm in $(ls -A $(eval "echo \$dr_pth_fn_${rnd}")); do
		if [[ -d $(eval "echo \$dr_pth_fn_${rnd}")/$dr_nm && -f $(eval "echo \$dr_pth_fn_${rnd}")/$dr_nm/g.pr ]]; then
			l_00_echo_info  "file://$(eval "echo \$dr_pth_fn_${rnd}")/$dr_nm/g.pr"
			cat $(eval "echo \$dr_pth_fn_${rnd}")/$dr_nm/g.pr | grep "doing ::"
		fi
	done
